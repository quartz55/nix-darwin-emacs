...
#+begin_quote
A nix overlay for nearly stable Emacs on Darwin (the heart of macOS).
#+end_quote

* Previews
[[file:./screenshots/preview.png]]

#+begin_quote
This screenshot demonstrates:
+ the =round-undecorated-frame.patch=
+ the customized application icon
#+end_quote

* About this overlay
** nearly stable Emacs
This overlay provides a package =emacs-darwin= which is:
+ built from the latest emacs package in nixpkgs.
...
* Treesitter
By default, the =emacs-darwin= package links all the =pkgs.tree-sitter-grammars.tree-sitter-*= packages, even though Emacs doesn't yet support all the corresponding ~-ts-mode~.
...
{ pkgs, ... }:
{
  environment.systemPackages = [
    (pkgs.emacsWithPackagesFromUsePackage {
      ...
      # Package is optional, defaults to pkgs.emacs-darwin
