...
* Overriding ~treeSitterPlugins~ attr
By default, the =emacsGit= package links all the =pkgs.tree-sitter-grammars.tree-sitter-*= packages, even though Emacs doesn't yet support all the corresponding ~-ts-mode~.

If you don't like it, you can try to override the ~treeSitterPlugins~ attr of ~emacsGit~.
